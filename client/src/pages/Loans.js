...
function AddLoan() {
  ...
  function onSubmit(values) {
    const { customerId, providerId, amount, interestRate, deadline } = values;

    const deadlineInUnixTime = new Date(deadline).valueOf();
    yamiChainContract.methods
      .addLoan(
        toUInt(customerId),
        toUInt(providerId),
        toUInt256(amount),
        toUInt256(interestRate),
        toUInt256(deadlineInUnixTime)
      )
  ...
  return (
    <Card title="Add Loan" boxProps={{ p: 6 }}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <FormControl mt={8} isRequired>
          <FormLabel>Customer ID</FormLabel>
          <Input ref={register} name="customerId" placeholder="Customer ID" />
        </FormControl>
        <FormControl mt={8} isRequired>
        ...
        <FormControl mt={8} isRequired>
          ...
          <Input ref={register} name="deadline" type="date" />
        </FormControl>
        <Button
          {...buttonStyles}
          mt={8}
